...
export function Header({ onToggleSidebar, showMenuButton = false }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-40 flex h-16 items-center gap-4 border-b border-border bg-background/80 backdrop-blur-md px-4 md:px-6">
      ...
      <Link href="/" className="flex items-center gap-2 font-bold">
        ...
        <span className="hidden sm:inline gradient-text text-lg">CommitCamp</span>
      </Link>

      <nav className="ml-auto flex items-center gap-1">
        <Button variant="ghost" size="icon" onClick={toggleLocale} title={t("language")}>
