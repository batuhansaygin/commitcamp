...
/**
 * SnippetsLiveUpdater — subscribes to new snippets via Supabase Realtime
 * and shows a non-disruptive "N new snippets · Click to load" banner.
 *
...
export function SnippetsLiveUpdater() {
  ...
  useEffect(() => {
    ...
    channelRef.current = supabase
      .channel("snippets-new")
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "snippets", filter: "is_public=eq.true" },
        () => {
          setNewCount((c) => c + 1);
          setDismissed(false);
          resetDismissTimer();
  ...
  return (
    <div className="flex justify-center">
      ...
      >
        ...
        <span>
          {newCount === 1 ? "1 new snippet" : `${newCount} new snippets`} · Click to load
        </span>
        <button
